...
struct minstrel_rate_prediction mrp;

/* initialize rate prediction algorithm */
void rate_prediction_init()
{
	int i;
	mrp.lrIndex = 0;
	for (i = 0; i < LINEAR_REGRESSION_ARRAY_SIZE; i++)
	{
		mrp.time[i] = 0;
		mrp.probs[i] = 0;
	}
...
int rate_prediction_predict(int time)
{
	int sumX = 0, sumX2 = 0, sumY = 0, sumXY = 0, a, b;
	int i;
	for (i = 1; i <= mrp.lrIndex; i++)
	{
		sumX = sumX + mrp.time[i];
		sumX2 = sumX2 + mrp.time[i] * mrp.time[i];
		sumY = sumY + mrp.probs[i];
		sumXY = sumXY + mrp.time[i] * mrp.probs[i];
	}
	b = (mrp.lrIndex * sumXY - sumX * sumY) / (mrp.lrIndex * sumX2 - sumX * sumX);
	a = (sumY - b * sumX) / mrp.lrIndex;
	return (int)(a + b * time);
}
		       ...
		       int prob_avg)
{
	...
	/* float not account throughput if sucess prob is below 10% */
	...
	if (mi->ampdu_packets > 0) {
		if (!ieee80211_hw_check(mp->hw, TX_STATUS_NO_AMPDU_LEN)){
			if (mrp.lrIndex < 2)
			{
				...
				/* save time */
				mrp.time[mrp.lrIndex % LINEAR_REGRESSION_ARRAY_SIZE] = (mrp.lrIndex % LINEAR_REGRESSION_ARRAY_SIZE);

				/* save probability */
				mrp.probs[mrp.lrIndex % LINEAR_REGRESSION_ARRAY_SIZE] = (int)(mi->avg_ampdu_len);

				/* increase index */
				mrp.lrIndex = ((mrp.lrIndex + 1) % LINEAR_REGRESSION_ARRAY_SIZE);
			}
			else
			{
				...
				/* calculate the prob */
				mrp.probs[mrp.lrIndex % LINEAR_REGRESSION_ARRAY_SIZE] = (int)rate_prediction_predict((int)((mrp.lrIndex + 1) % LINEAR_REGRESSION_ARRAY_SIZE));

				/* save probability */
				mi->avg_ampdu_len = mrp.probs[(mrp.lrIndex % LINEAR_REGRESSION_ARRAY_SIZE)];
			...
			struct ieee80211_sta *sta, void *priv_sta)
{
	...
	/* initialize rate prediction */
	rate_prediction_init();
